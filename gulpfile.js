...
gulp.task('scss', function () {
    return gulp.src([paths.src.scss + '/neu/**/*.scss', paths.src.scss + '/neu.scss'])
        .pipe(wait(500))
...
gulp.task('serve', gulp.series('scss', 'html', 'index', 'assets', 'vendor', function() {
    ...
    gulp.watch([paths.src.scss + '/neu/**/*.scss', paths.src.scss + '/neu.scss'], gulp.series('scss'));
    gulp.watch([paths.src.html, paths.src.base + '**.html', paths.src.partials], gulp.series('html', 'index'));
    gulp.watch([paths.src.docs, paths.src.base + '**.html', paths.src.docs], gulp.series('docs', 'index'));
...
// Minify Docs
gulp.task('minify:docs', function () {
    return gulp.src([paths.dist.docs + '/**/*.html'])
        .pipe(htmlmin({
            collapseWhitespace: true
        }))
        .pipe(fileinclude({
...
gulp.task('minify:docs:index', function () {
    return gulp.src([paths.dist.base + '*.html'])
        .pipe(htmlmin({
            collapseWhitespace: true
        }))
...
gulp.task('copy:dev:css', function () {
    return gulp.src([paths.src.scss + '/neu/**/*.scss', paths.src.scss + '/neu.scss'])
        ...
        .pipe(sourcemaps.write('.'))
        .pipe(gulp.dest(paths.dev.css))
        .pipe(gulp.dest(paths.dev.docs + '/css'))
});
...
// Copy docs index
gulp.task('copy:dist:docs:index', function () {
    return gulp.src([paths.src.base + '**.html'])
        .pipe(fileinclude({
            prefix: '@@',
            basepath: './src/partials/',
...
gulp.task('copy:dev:docs:index', function () {
    return gulp.src([paths.src.base + '**.html'])
        .pipe(fileinclude({
            prefix: '@@',
            basepath: './src/partials/',
...
gulp.task('copy:dist:assets', function () {
    return gulp.src(paths.src.assets)
        ...
        .pipe(gulp.dest(paths.dist.docs + '/assets'))
});
...
gulp.task('copy:dist:vendor', function() {
    return gulp.src(npmDist(), { base: paths.src.node_modules })
      ...
      .pipe(gulp.dest(paths.dist.docs + '/vendor'));
});

gulp.task('copy:dev:vendor', function() {
    return gulp.src(npmDist(), { base: paths.src.node_modules })
      .pipe(gulp.dest(paths.dev.vendor))
      .pipe(gulp.dest(paths.dev.docs + '/assets'));
});

gulp.task('build:dev', gulp.series('clean:dev', 'copy:dev:css', 'copy:dev:html', 'copy:dev:docs', 'copy:dev:html:index', 'copy:dev:docs:index', 'copy:dev:assets', 'beautify:css', 'copy:dev:vendor'));
gulp.task('build:dist', gulp.series('clean:dist', 'copy:dist:css', 'copy:dist:html','copy:dist:docs', 'copy:dist:html:index', 'copy:dist:docs:index', 'copy:dist:assets', 'minify:css', 'minify:html', 'minify:docs', 'minify:html:index', 'minify:html:index', 'copy:dist:vendor'));
